...
import {readScreenshotJson} from './screenshotReader'
...
export async function run(): Promise<void> {
  try {
    ...
    const screenshotResult = await readScreenshotJson(screenshotPaths)
    ...
    if (jobSummary && supportsJobSummary) {
      try {
        await attachSummary(testResults, detailedSummary, includePassed, screenshotResult.screenshots)
      } catch (error) {
